        ...
        switch (data.type) {
          ...
          case 'TRANSLATED_AUDIO':
            ...
            try {
              ...
              const chunk = {
                id: `translated_${data.messageId || Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
                data: data.audioData,
                timestamp: data.timestamp || Date.now(),
                responseId: data.messageId || 'unknown'
              };
          ...
          case 'AUDIO_CHUNK':
            console.log(' Received audio chunk:', data.chunkId || 'chunk');
            break;
